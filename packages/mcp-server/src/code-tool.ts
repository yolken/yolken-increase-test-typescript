...
import { ClientOptions } from 'yolken-increase-test';

const prompt = `Runs JavaScript code to interact with the Yolken Increase Test API.
...
const remoteStainlessHandler = async (
  ...
  const localClientEnvs = { YOLKEN_INCREASE_TEST_API_KEY: requireValue(readEnv('YOLKEN_INCREASE_TEST_API_KEY') ?? client.apiKey, 'set YOLKEN_INCREASE_TEST_API_KEY environment variable or provide apiKey client option'), YOLKEN_INCREASE_TEST_BASE_URL: readEnv('YOLKEN_INCREASE_TEST_BASE_URL') ?? readEnv('YOLKEN_INCREASE_TEST_ENVIRONMENT') ? undefined : client.baseURL ?? undefined };
  ...
  const res = await fetch(codeModeEndpoint, {
    ...
    body: JSON.stringify({
      project_name: "yolken-increase-test",
      code,
      intent,
      client_opts: { environment: (readEnv('YOLKEN_INCREASE_TEST_ENVIRONMENT') || undefined) as any },
...
const localDenoHandler = async (
  ...
  try {
    const sdkPkgName = 'yolken-increase-test';
