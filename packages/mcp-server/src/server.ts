...
export async function initMcpServer(params: {
  ...
  let _client: YolkenIncreaseTest | undefined;
  let _clientError: Error | undefined;
  let _logLevel: 'debug' | 'info' | 'warn' | 'error' | 'off' | undefined;

  const getClient = (): YolkenIncreaseTest => {
    if (_clientError) throw _clientError;
    if (!_client) {
      try {
        _client = new YolkenIncreaseTest({
          ...{ environment: (readEnv('YOLKEN_INCREASE_TEST_ENVIRONMENT') || undefined) as any },
          logger,
