...
export class CheckDeposits extends APIResource {
   ...
   * @example
   * ```ts
   * const checkDeposits = await client.checkDeposits.list();
   * ```
   */
  list(query: CheckDepositListParams | null | undefined = {}, options?: RequestOptions): APIPromise<CheckDepositListResponse> {
    return this._client.get('/check_deposits', { query, ...options });
  }
}
...
export interface CheckDeposit {
   ...
   */
  inbound_funds_hold: ACHTransfersAPI.InboundFundsHold | null;
