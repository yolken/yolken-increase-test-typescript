...
describe('resource bookkeepingEntrySets', () => {
  ...
  test.skip('create: only required params', async () => {
    const responsePromise = client.bookkeepingEntrySets.create({ entries: [{ account_id: 'bookkeeping_account_9husfpw68pzmve9dvvc7', amount: 100 }, { account_id: 'bookkeeping_account_t2obldz1rcu15zr54umg', amount: -100 }] });
  ...
  test.skip('retrieve', async () => {
    const responsePromise = client.bookkeepingEntrySets.retrieve('bookkeeping_entry_set_n80c6wr2p8gtc6p4ingf');
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
  ...
  test.skip('list: request options and params are passed correctly', async () => {
    ...
    await expect(client.bookkeepingEntrySets.list({
    cursor: 'cursor',
    idempotency_key: 'x',
    limit: 1,
